...
import { getProductById, Product } from '@/api/store';
...
export function ProductDetail() {
  ...
  const numeroWhatsApp = '5583988083711'; // Substitua pelo número real
  let whatsappUrl = '';
  if (product) {
    const mensagem = `Olá! Tenho interesse no produto: *${product.name}* (ID: ${product._id}).`;
    whatsappUrl = `https://wa.me/${numeroWhatsApp}?text=${encodeURIComponent(mensagem)}`;
  }
                ...
                {product.images.map((image, index) => (
                  <button
                    ...
                    className={`rounded-lg overflow-hidden border-2 transition-all ${
                      index === currentImageIndex ? 'border-pink-500 scale-105' : 'border-transparent opacity-70 hover:opacity-100'
                    }`}
                  >
